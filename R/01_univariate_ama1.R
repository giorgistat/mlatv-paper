...
#   AMA1 (Section 5.1 of the paper). Adapted from example28_univariate_ama1.R.
...
#     reproduce the paper's AMA1 estimates -- only to exercise the pipeline.
#     When re-running on the real data, restore your own start values.
# Output: output/rds/01_univariate_ama1.rds, output/plot/01_ama1_age_histograms.pdf
...
if (!is.finite(fit_nonspatial$logLik))
  stop("AMA1 non-spatial fit returned a non-finite log-likelihood.")
if (fit_nonspatial$convergence != 0L)
  warning("AMA1 non-spatial fit did not converge: ", fit_nonspatial$message)
...
if (!is.finite(fit_spatial$logLik))
  stop("AMA1 spatial fit returned a non-finite log-likelihood.")
if (fit_spatial$convergence != 0L)
  warning("AMA1 spatial fit did not converge: ", fit_spatial$message)

# ---- Report -----------------------------------------------------------------
fit_dimensions <- data.frame(
  response = "AMA1", observations = nrow(analysis_data),
...
plot_univariate_ppc_grid(
  ...
  beta_hat = fit_spatial$par[["beta"]], seed0 = 20260101L
)
grDevices::dev.off()
cat("Saved:", fig_hist, "\n")
...
saveRDS(
  ...
  file.path(rds_dir, "01_univariate_ama1.rds")
)
cat("\nSaved: output/rds/01_univariate_ama1.rds\n")
